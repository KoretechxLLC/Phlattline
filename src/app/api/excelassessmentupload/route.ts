...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, message: 'Assessments created successfully.' }, { status: 201 });
  } catch (error: any) {
    console.error('Error creating assessments:', error);
    return NextResponse.json(
      { error: error?.message || 'Failed to create assessments', details: error.message },
      { status: 500 }
    );
  }
}
